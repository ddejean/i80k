 ...
 */
typedef void (*xmodem_tx_byte)(struct xmodem_server *xdm, uint8_t byte,
                               void *cb_data);
...
struct xmodem_server {
    ...
    uint16_t packet_pos;        // Where are we up to in this packet
    ...
    uint64_t last_event_time;  // When did we last do something interesting?
 ...
 */
int xmodem_server_init(struct xmodem_server *xdm, xmodem_tx_byte tx_byte,
                       void *cb_data);

/**
 * Send a single byte to the xmodem state machine
 * @returns true if a packet is now available for processing, false if more data
 * is needed
 */
 ...
 * @param packet Area to store the next decoded packet. Must be at least
 * XMODEM_MAX_PACKET_SIZE long. xdm->packet_size bytes will be copied in here
 ...
 * @return Number of bytes of data copied into 'packet' (either 128, or 1024),
 * or 0 if no new packet is available
 */
int xmodem_server_process(struct xmodem_server *xdm, uint8_t *packet,
                          uint32_t *block_num, uint64_t ms_time);
