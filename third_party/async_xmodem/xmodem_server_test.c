...
static bool rx_packet(struct xmodem_server *xdm, const uint8_t *data,
                      int data_len, int block_nr, int error_rate) {
    uint16_t crc = 0;
    uint8_t tx_type;
    if (data_len == 128)
        tx_type = 0x01;
    ...
    for (int i = 0; i < data_len; i++) {
        uint8_t b = data[i];
        if (error_rate && (rand() % error_rate) == 0) b ^= rand() & 0xff;
...
static void test_timeout(void) {
    ...
    // We expect a packet every second, and currently limit ourselves to 10
    // errors so after 11 seconds we should be broken
...
static void test_sz(bool use_1k, size_t data_size) {
    ...
    fclose(fp);
    char *const args_1k[] = {"sz",      "--xmodem",    "--1k",
                             "--quiet", raw_data_name, NULL};
    ...
    while (!xmodem_server_is_done(&xdm)) {
        ...
        if (select(max_fd + 1, &rd_fds, &wr_fds, NULL, &tv) >= 0) {
            ...
            data_len = xmodem_server_process(&xdm, resp, &block_nr, ms_time());
            if (data_len > 0) {
                memcpy(&output_data[block_nr * xdm.packet_size], resp,
                       data_len);
            }
        }
    }
...
static void test_sz_128(void) { test_sz(false, 2 * 1024 * 1024); }

static void test_sz_1k(void) { test_sz(true, 2 * 1024 * 1024); }

TEST_LIST = {
    {"simple", test_simple},   {"errors", test_errors},
    {"timeout", test_timeout}, {"sz (128B)", test_sz_128},
    {"sz (1kB)", test_sz_1k},  {NULL, NULL},
};
